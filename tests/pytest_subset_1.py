...
def test_multiple_commands_63_corrected(capsys):
    args = ['-n', '1,/.1/p;/5/,/9/s/.//;/.2/,/.9/p;85q']
...
def test_multiple_commands_from_file_same_as_63(capsys, tmp_path):
    # 写入和上一测试相同的命令到临时命令文件中
    cmd_file = tmp_path / "commandsFile"
    cmd_file.write_text("1,/.1/p;/5/,/9/s/.//\n/.2/,/.9/p;85q")

    args = ['-n', '-f', str(cmd_file)]
    stdin = "\n".join(str(i) for i in range(1, 101)) + "\n"
