...
def input_generator(input_file=None):
...
def run_pied(i_flag: bool, n_flag: bool, cmd_lst: list, input_file=None):
    ...
    for line_num, two_lines in enumerate(input_gen):
        in_range = False
...
def run(cmd_lst, cur_line, line_num, is_last, n_flag, temp_file) -> tuple[str, bool]:
    """
...
def run_cmd(cur_line, cmd_lst, i, line_num, is_last, output_flags, n_flag, temp_file):
    ...
    """
    if not cur_line:
        return None, i + 1, output_flags
    cmd = cmd_lst[i]
    addr1 = cmd["addr1"]
    addr2 = cmd["addr2"]
    if aadr_matchs(cur_line, line_num, is_last, addr1, addr2):
        match cmd["cmd"]:
        # 识别 cmd 字段，执行不同指令逻辑:
        # 修改 result -> 修改 next_i -> 修改 changed
            case "p":
                output(n_flag, cur_line, [], temp_file)
                i += 1
            case "q":
...
def aadr_matchs(cur_line: str, line_num: int, is_last: bool, addr1, addr2) -> bool:
    ...
    def match(addr, line_num, is_last):
        ...
        if addr == '$':
            ...
            return int(addr) == line_num + 1  # 注意 sed 行号从 1 开始
    ...
    if addr2 is None:
        ...
        in_range = False
        if not hasattr(aadr_matchs, "_range_active"):
            aadr_matchs._range_active = False

        if not aadr_matchs._range_active:
            if match(addr1, line_num, is_last):
                aadr_matchs._range_active = True
                in_range = True
        else:
            in_range = True
